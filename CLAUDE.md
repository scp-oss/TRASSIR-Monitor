...
через SOCKS5 и через "воркер" (свой relay/worker с адресом + ключом
авторизации), по образцу `<адрес>/bot<токен>/<метод>?auth=<ключ>` вместо
`api.telegram.org/bot<токен>/<метод>`. Пример в исходном запросе был с
конкретным чужим доменом-примером и код на нескольких языках, все
использующие тот же домен.

**Сознательное решение: этот (или любой другой) конкретный чужой домен
никогда не зашивается в код по умолчанию.** Токен Telegram-бота —
полноценный секрет (полный контроль над ботом), и он уходит открытым
текстом в URL при каждой отправке. Зашитый по умолчанию адрес
непроверенного стороннего relay — это тихий путь утечки/угона токена в
публичном репозитории: любой, кто читает исходники (а их читает кто
угодно — репозиторий публичный), увидел бы, куда токен реально уходит,
и мог бы поднять тот же relay под собой. Та же логика уже применена к
HTTP-прокси в `install-telegram-notifier.sh` — прокси **всегда** вводит
сам пользователь при установке, никогда не задан по умолчанию; workers
устроены так же, просто с полем ввода в `/settings`, а не при установке.
Реализована именно та функциональность, что просили (три реальных
работающих способа отправки), только адрес/ключ для варианта "workers"
— **произвольные значения, которые вводит сам администратор**, никогда
не пример из чата.

**Где это живёт:**
- `install-telegram-notifier.sh` (`tg_bot.py`): `get_config()` теперь
  читает не только `config.ini` (`token`/`proxy`/`monitor_url`, как
  раньше), но и переопределения способа отправки из таблицы БД
  `telegram_settings` (`get_db_transport_overrides()`) — `proxy_url`
  (перекрывает `proxy` из config.ini), `api_base`, `api_key`. Читаются
  заново на каждую отправку — переключение способа в `/settings`
  применяется без перезапуска службы и без переустановки бота.
  `_telegram_api_url(cfg, method)` строит итоговый URL: пусто в
  `api_base` → обычный `https://api.telegram.org/bot<token>/<method>`;
  иначе → `<api_base>/bot<token>/<method>` (+ `?auth=<api_key>` если
  ключ задан). `get_proxies()` не изменился по коду — `requests`
  генерически поддерживает схему `socks5://`/`socks5h://` в `proxies=`,
  ей нужен только установленный пакет `PySocks` в venv (раньше не
  ставился — добавлен в `install-trassir-monitor.sh`'s pip install,
  сразу после `requests`, тем же venv пользуется и `tg_bot.py`).
- `install-trassir-monitor.sh` (`app.py`): `/api/telegram/settings` GET
  добавляет `proxy_url_configured`/`proxy_url_masked`,
  `api_key_configured`/`api_key_masked`, `proxy_masked` (маскированный
  `proxy` из config.ini, тем же приёмом что и `token_masked`) — **сырые
  `proxy_url`/`api_key` никогда не возвращаются в JSON**, только факт
  настройки + маска (`login:***@host`/`abcd...wxyz`), тот же принцип что
  уже был у `token_masked`. POST по-прежнему пишет произвольные
  key/value в `telegram_settings` (ничего нового не потребовалось), но
  теперь валидирует схему `proxy_url` (`http(s)/socks5(h)://...`) и
  `api_base` (`http(s)://...`) перед записью — отклоняет с понятной
  ошибкой вместо тихой поломки отправки задним числом. Settings-форма
  трактует пустое поле `api_key`/`proxy_url` как "не менять" (тот же
  UX-принцип, что уже применяется к паролю администратора и паролям
  серверов при экспорте/импорте) — реально стереть значение можно только
  явным переключением на "Напрямую" (шлёт пустые строки во все три поля).
- `templates/settings.html`: карточка Telegram получила блок "Способ
  отправки" — три радиокнопки (Напрямую / Прокси / Свой relay) с полями,
  видимость которых переключается на клиенте (`tgModeChanged()`), и
  `saveTgTransport()` собирает и шлёт только относящиеся к выбранному
  режиму поля.

Протестировано симуляцией (тот же подход, что и везде в этом проекте —
`py_compile` + извлечение heredoc'ов + `test_client()`/прямой вызов
функций, без реального сервера): все три режима в `tg_bot.py`
(`_telegram_api_url`), маскировка/невозможность утечки сырых
`proxy_url`/`api_key` через GET (анонимный и авторизованный доступ),
валидация схем при POST, и рендер трёх режимов в JS (`renderTgTransport`
