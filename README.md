...
В шапке меню — дата изменения самого запущенного файла на диске и, если
лаунчер запущен из настоящего `git clone` этого репозитория (а не
скачан одним файлом через `wget`, как в примере выше), короткий хеш
текущего коммита. При обычном способе установки (`wget` одного файла)
хеш коммита показывается как `?` — это ожидаемо, git-информации там
взять неоткуда.
...
коммит: ...`), тот же принцип, что и в шапке `launcher-trassir-monitor.sh`
(см. раздел про лаунчер выше) — помогает быстро понять, действительно ли
на сервере уже применилось последнее обновление. Хеш коммита показывается
только если `/opt/trassir-monitor` неожиданно оказался настоящим git-чекаутом
(при обычной установке — никогда, там просто `?`, это ожидаемо).
