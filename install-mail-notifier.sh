...
# переживает переустановки Email-бота отдельно от дашборда).
echo ""
...
COMMIT_ERROR_FILE="$INSTALL_DIR/data/.installed_commit_mail.error"
if [ -n "$LATEST_COMMIT" ]; then
    echo "$LATEST_COMMIT" > "$INSTALL_DIR/data/.installed_commit_mail"
